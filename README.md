...
ComfyUI Nodes 2.0 introduced a new widget rendering architecture that fundamentally changed how custom UI nodes display. All custom UI nodes in Internode (Audio Mixers, Studio Surface, Sticky Notes, Markdown) have been updated to work seamlessly in both modes:

*   **What Changed:** Nodes 2.0 renders ALL widgets in the `node.widgets` array, ignoring visibility flags that worked in Nodes 1.0
*   **Our Solution:** Custom UI nodes now use widget array manipulation - hidden widgets are removed from the array and stored separately with custom accessor functions
*   **Result:** Clean, professional UI with no parameter clutter in either Nodes 1.0 or 2.0
